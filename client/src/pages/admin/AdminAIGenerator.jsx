...
import { Wand2, Download, AlertCircle, RefreshCw, UploadCloud, Image } from 'lucide-react';
...
const AdminAIGenerator = () => {
  ...
  const handleGenerate = async (e) => {
    e.preventDefault();
    if (!prompt.trim()) {
      toast.error('Please enter a prompt to generate the PNG.');
      return;
    }
    ...
    try {
      const res = await api.post('/ai/generate-png', { prompt: prompt.trim() });
      setPreviewBase64(res.data.data);
      toast.success('AI PNG generated successfully! Please review it.');
    } catch (err) {
      console.error(err);
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      formData.append('runAI', 'true'); // Auto generate tags & description using existing flow
    ...
    <AdminLayout title="AI PNG Generator">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
            ...
            <form onSubmit={handleGenerate} className="space-y-4">
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  placeholder="e.g. Create a high-quality transparent PNG of a roaring white tiger. Ultra detailed, centered object, isolated subject..."
                  rows="4"
              ...
              >
                {isGenerating ? (
                  <><RefreshCw className="animate-spin h-4 w-4 mr-2" /> Generating via DALL-E 3...</>
                ) : (
                  <><Wand2 className="w-4 h-4 mr-2" /> Generate PNG</>
                )}
              </button>
            </form>
          </div>
          ...
          {previewBase64 && (
            <div className="glass p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-slate-850 bg-white/80 dark:bg-slate-900/10 space-y-6">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">Publish Settings</h2>
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    placeholder="White Tiger Roaring"
              ...
              <div className="p-4 bg-brand-500/10 border border-brand-500/20 rounded-2xl flex items-start space-x-3">
                ...
                <p className="text-xs text-brand-600 dark:text-brand-400">
                  SEO Tags and Description will be automatically generated by AI during the publish process.
                </p>
              </div>
              ...
              >
                {isPublishing ? (
                  ...
                  <><UploadCloud className="w-4 h-4 mr-2" /> Publish Generated PNG</>
                )}
              </button>
            </div>
          )}
          ...
          <div className="glass rounded-3xl border border-slate-200 dark:border-slate-850 bg-white/80 dark:bg-slate-900/10 overflow-hidden sticky top-24 min-h-[350px] flex flex-col justify-center items-center">
            {isGenerating ? (
              <div className="flex flex-col items-center justify-center space-y-4 p-8 text-center">
                ...
                <p className="text-sm font-bold text-slate-900 dark:text-white">AI is painting...</p>
                <p className="text-xs text-slate-500">This might take 10-20 seconds.</p>
              </div>
            ) : previewBase64 ? (
              <div className="w-full h-full p-4 bg-checkerboard bg-[size:10px_10px] flex items-center justify-center border-b border-slate-200 dark:border-slate-800">
                <img src={previewBase64} alt="AI Generated Preview" className="max-w-full max-h-[60vh] object-contain drop-shadow-2xl rounded-lg" />
              </div>
            ) : (
