...
const PngCard = ({ png, onDownloadSuccess, rank }) => {
  ...
  return (
    ...
    >
      {rank && rank <= 3 && (
        <div className={`absolute top-0 left-0 m-3 z-20 flex items-center justify-center w-8 h-8 rounded-full border backdrop-blur-md shadow-lg transition-opacity duration-300 group-hover:opacity-0 font-black text-sm tracking-tighter ${rankIconColor}`}>
          #{rank}
        </div>
      )}
          ...
          <div className="absolute inset-0 bg-slate-900/80 opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-between p-5 text-white">
            ...
            <div className="flex justify-between items-start transform translate-y-[-10px] group-hover:translate-y-0 transition-transform duration-300">
              ...
              <button
                ...
                className={`shrink-0 p-3 rounded-2xl bg-white/10 hover:bg-white/20 backdrop-blur-md text-white border border-white/20 shadow-lg transition-all ${
